...
import { z } from "zod";
import { createGrafanaService } from "./grafanaService";

export async function registerRoutes(app: Express): Promise<Server> {
  const grafanaService = createGrafanaService(storage);
  ...
  app.patch("/api/services/:id/status", async (req, res) => {
    try {
      ...
      if (!status || !["operational", "degraded", "down", "maintenance", "loading"].includes(status)) {
  ...
  app.get("/api/grafana/status", async (req, res) => {
    try {
      const configured = grafanaService.isConfigured();
      res.json({ 
        configured,
        url: configured ? process.env.GRAFANA_URL : null,
        message: configured ? "Grafana is configured" : "Grafana is not configured"
      });
    } catch (error) {
      res.status(500).json({ error: "Failed to check Grafana status" });
    }
  });

  app.post("/api/grafana/sync", async (req, res) => {
    try {
      if (!grafanaService.isConfigured()) {
        return res.status(400).json({ error: "Grafana is not configured" });
      }

      const result = await grafanaService.syncServiceStatuses();
      res.json({ 
        success: true, 
        ...result,
        message: `Synced ${result.updated} services from Grafana`
      });
    } catch (error) {
      console.error("Grafana sync error:", error);
      res.status(500).json({ 
  ...
  app.get("/api/grafana/metrics", async (req, res) => {
    try {
      if (!grafanaService.isConfigured()) {
        return res.status(400).json({ error: "Grafana is not configured" });
      }

      const query = (req.query.query as string) || 'up{job="node_exporter"}';
      const metrics = await grafanaService.fetchMetrics(query);
